...
if [ "$fancy_prompt" = yes ]; then
    function ps1_c {
        ...
        dir=${dir#"$HOME/"}
        dir=${dir%/}
        printf "\e]0;%s:%s\a\e[37m┌─\e[m %s\e[1;32m%s@%s\e[m:\e[1;34m%s\e[m\n" "$HOSTNAME" "$dir" "${debian_chroot:+($debian_chroot)}" "$USER" "$HOSTNAME" "$dir"
    ...
    PS1='\[\e[1m\]\$\[\e[m\] '
    PS2='> '
    PS0='\e[$((_hypercube_ps0_start=SECONDS,37))m├─\e[m[\e[36m\D{%Y-%m-%d %H:%M:%S}\e[m]\n'
else
